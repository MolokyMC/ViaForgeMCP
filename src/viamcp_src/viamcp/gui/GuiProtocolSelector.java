...
import viamcp.common.ViaMCPCommon;
...
public class GuiProtocolSelector extends GuiScreen {
    ...
    public GuiProtocolSelector(final GuiScreen parent) {
        this(parent, false, (version, unused) -> {
            ...
            ViaMCPCommon.getManager().setTargetVersion(version);
        });
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRendererObj, ChatFormatting.GOLD + "ViaForgeMCP", width / 4, 3, 16777215);
        GL11.glPopMatrix();

        drawCenteredString(fontRendererObj, "https://github.com/MolokyMC/ViaForgeMCP", width / 2, (fontRendererObj.FONT_HEIGHT + 2) * 2 + 3, -1);
        drawString(fontRendererObj, status != null ? status : "bilibili: 泠羽微光", 3, 3, -1);
    ...
    class SlotList extends GuiSlot {
        ...
        protected void drawSlot(int index, int x, int y, int slotHeight, int mouseX, int mouseY) {
            final ProtocolVersion targetVersion = ViaMCPCommon.getManager().getTargetVersion();
