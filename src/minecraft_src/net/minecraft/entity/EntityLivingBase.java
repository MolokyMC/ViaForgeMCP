...
public abstract class EntityLivingBase extends Entity
{
    ...
    public void onLivingUpdate()
    {
        ...
        // ViaForgeMCP
        if (Math.abs(this.motionX) < VersionFixer.movementFix(0.005D))
        {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < VersionFixer.movementFix(0.005D))
        {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < VersionFixer.movementFix(0.005D))
        {
            this.motionZ = 0.0D;
        }
        // End
